...
# h7.popen(f"tcpdump -i {h7.defaultIntf()} -w /tmp/h7_capture_a_{congestion_control}.pcap &") # I start the tcpdump on h7
h1 = net.get('h1') # I get the host h1
# h1.popen(f"tcpdump -i {h1.defaultIntf()} -w /tmp/h1_capture_a_{congestion_control}.pcap &") # I start the tcpdump on h1
...
process = h1.popen(iperf_command, shell=True)
...
os.system('mkdir -p ./pcaps_captured_trail') # I create a directory to store the captured pcaps
os.system('mv /tmp/*.pcap ./pcaps_captured_trail/') # I move the pcaps to the directory
